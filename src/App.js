...
const ORDER_CODE_FILTER_OPTIONS = [
  { value: "", label: "Tất cả" },
];
...
export default function App() {
  ...
  const orderCodeOptions = useMemo(() => {
    const codes = Array.from(new Set(orders.map(order => order.order_code).filter(Boolean)));
    codes.sort((a, b) => String(a).localeCompare(String(b), "vi", { numeric: true }));
    return [
      ...ORDER_CODE_FILTER_OPTIONS,
  ...
  const filteredOrders = useMemo(() => {
    const filtered = orders.filter(o => {
      ...
      if (filterOrderCode && o.order_code !== filterOrderCode) return false;
    ...
    return sorted;
  }, [orders, customers, transactions, search, filterPayment, filterShipping, filterOrderCode, filterShippingFee, orderSort]);
  ...
  const hasFilters = filterPayment || filterShipping || filterOrderCode || filterShippingFee;
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        ...
        {tab === "orders" && (
          <div className="flex flex-wrap items-center gap-2 mb-4">
            ...
            <FilterDropdown label="Mã đơn" icon={IconClipboard} value={filterOrderCode} options={orderCodeOptions} onChange={setFilterOrderCode} />
            ...
            {hasFilters && (
              <button onClick={() => { setFilterPayment(""); setFilterShipping(""); setFilterOrderCode(""); setFilterShippingFee(""); }} className="flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-medium text-red-600 hover:bg-red-50 cursor-pointer">
